...
public class ActionResultCallback<TArg1, TResult>
{
    ...
    private Func<TArg1, Task<TResult>> _callback;
    ...
    public ActionResultCallback(
        Func<TArg1, Task<TResult>> callback
    )
    {
        _callback = callback;
    ...
    [JSInvokable]
    public async Task<TResult> HandleCallback(TArg1 arg1)
    {
        Console.WriteLine("HandleCallback:" + arg1);
        var result = await _callback(arg1);
        Console.WriteLine("HandleCallback:" + result);
        return result;
    }
}
